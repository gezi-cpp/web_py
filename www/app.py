...
import asyncio,os,json,time,jinja2,ORM
...
"""
def index(request):
    return web.Response(body='<h1>Awesome python web by花</h1>',content_type='text/html',charset='utf-8')
""" 
#记录URL日志的logger
...
async def init():
    await ORM.create_pool(loop=loop,host='127.0.0.1',port=3306,user='www-data',password='www-data',db='web_python')
    app=web.Application(middlewares=[
        logger_factory,auth_factory,response_factory
    ])#创建一个web服务器实例
    #HTTP请求 GET仅请求资源，POST会附带用户数据
...
loop.run_until_complete(init()) #执行协程init
loop.run_forever()        
